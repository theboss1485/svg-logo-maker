...
/*For the Shape class, I test a created Shape object to make sure that it is actually a Shape object.
I then test the fill color on two different shape objects and the setColor() method on those two Shape
objects.  I also test the XPosition and YPosition properties to make sure they are correct.  I test
...
tested the Triangle class's width and height properties. For the render() method test, I tested that 
the Triangle class returns the proper <polygon> element, rather than just throwing an error.  I also ran the example test from the
